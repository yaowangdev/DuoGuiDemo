 ...
 */
package com.appdev.framework.db.liteorm;

import android.content.Context;

import com.appdev.framework.db.liteorm.db.DataBase;
import com.appdev.framework.db.liteorm.db.DataBaseConfig;
import com.appdev.framework.db.liteorm.db.TableManager;
import com.appdev.framework.db.liteorm.db.assit.Checker;
import com.appdev.framework.db.liteorm.db.assit.CollSpliter;
import com.appdev.framework.db.liteorm.db.assit.Querier;
import com.appdev.framework.db.liteorm.db.assit.QueryBuilder;
import com.appdev.framework.db.liteorm.db.assit.SQLBuilder;
import com.appdev.framework.db.liteorm.db.assit.SQLStatement;
import com.appdev.framework.db.liteorm.db.assit.SQLiteHelper;
import com.appdev.framework.db.liteorm.db.assit.WhereBuilder;
import com.appdev.framework.db.liteorm.db.impl.CascadeSQLiteImpl;
import com.appdev.framework.db.liteorm.db.impl.SingleSQLiteImpl;
...
import com.appdev.framework.db.liteorm.db.model.EntityTable;
import com.appdev.framework.db.liteorm.db.model.MapProperty;
import com.appdev.framework.db.liteorm.db.model.RelationKey;
import com.appdev.framework.db.liteorm.db.utils.ClassUtil;
import com.appdev.framework.db.liteorm.db.utils.DataUtil;
import com.appdev.framework.db.liteorm.db.utils.FieldUtil;
...
public abstract class LiteOrm extends SQLiteClosable implements DataBase {
    ...
    protected LiteOrm(DataBaseConfig config) {
        ...
        SQLiteDatabase.loadLibs(config.context);
    ...
    public SQLiteDatabase openOrCreateDatabase() {
        ...
        mTableManager = new TableManager(mConfig.dbName, mHelper.getReadableDatabase(mConfig.password));
        return mHelper.getWritableDatabase(mConfig.password);
    }
     ...
     */
    public static LiteOrm newSingleInstance(Context context, String dbName, String password) {
        return newSingleInstance(new DataBaseConfig(context, dbName, password));
    }
     ...
     * @return {@link SingleSQLiteImpl}
     */
    public static LiteOrm newCascadeInstance(Context context, String dbName, String password) {
        return newCascadeInstance(new DataBaseConfig(context, dbName, password));
    }
            ...
            if (mTableManager.isSQLMapTableCreated(table1.name, table2.name)) {
                CollSpliter.split(key1List, SQLStatement.IN_TOP_LIMIT, new CollSpliter.Spliter<String>() {
                    ...
                    public int oneSplit(ArrayList<String> list) throws Exception {
                        ...
                        Querier.doQuery(mHelper.getReadableDatabase(mConfig.password), stmt, new Querier.CursorParser() {
    ...
    public boolean dropTable(String tableName) {
        acquireReference();
        try {
            return SQLBuilder.buildDropTable(tableName).execute(mHelper.getWritableDatabase(mConfig.password));
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            releaseReference();
    ...
    public synchronized SQLiteDatabase getReadableDatabase() {
        return mHelper.getReadableDatabase(mConfig.password);
    }
    ...
    public SQLiteDatabase openOrCreateDatabase(String path, SQLiteDatabase.CursorFactory factory) {
        path = mConfig.context.getDatabasePath(mConfig.dbName).getPath();
        return SQLiteDatabase.openOrCreateDatabase(path, mConfig.password, factory);
    }

    @Override
    public boolean deleteDatabase() {
        String path = mHelper.getWritableDatabase(mConfig.password).getPath();
        justRelease();
    ...
    protected void justRelease() {
        if (mHelper != null) {
            mHelper.getWritableDatabase(mConfig.password).close();
            mHelper.close();
            mHelper = null;
        }
