...
public final class SingleSQLiteImpl extends LiteOrm {
    ...
    public <T> int delete(final Collection<T> collection) {
        ...
        try {
            if (!Checker.isEmpty(collection)) {
                ...
                if (mTableManager.isSQLTableCreated(table.name)) {
                    int rows;
                    final SQLiteDatabase db = mHelper.getWritableDatabase(mConfig.password);
                    db.beginTransaction();
                    try {
    ...
    public <T> ArrayList<T> query(QueryBuilder<T> qb) {
        ...
        if (mTableManager.isSQLTableCreated(table.name)) {
            acquireReference();
            try {
                return qb.createStatement().query(mHelper.getReadableDatabase(mConfig.password), qb.getQueryClass());
            } finally {
                releaseReference();
            }
        } else {
        ...
        if (mTableManager.isSQLTableCreated(table.name)) {
            ...
            try {
                SQLStatement stmt = new QueryBuilder<T>(claxx)
                        ...
                        .createStatement();
                ArrayList<T> list = stmt.query(mHelper.getReadableDatabase(mConfig.password), claxx);
